...
(function (global) {
  ...
  function renderGenerations(container) {
    var PAGE = 25;
    var offset = 0;
    var filters = { feature_type: '', from_date: '', to_date: '', model: '', search: '' };
    
    container.innerHTML = '<div class="admin-page-head"><h1 class="admin-page-title">AI Generations</h1><p class="admin-page-desc">Track all AI generations across scripts, thumbnails, and more. Filter by user, feature type, date, and model.</p></div>' +
      '<div class="admin-toolbar admin-toolbar--glassy">' +
        '<input type="text" id="gens-search" class="admin-input" placeholder="Search by user email...">' +
        '<select id="gens-type" class="admin-input"><option value="">All Types</option><option value="script">Scripts</option><option value="thumbnail">Thumbnails</option><option value="thumbnail_rating">Thumbnail Rating</option><option value="thumbnail_improve">Thumbnail Improve</option></select>' +
        '<input type="date" id="gens-from" class="admin-input" placeholder="From date">' +
        '<input type="date" id="gens-to" class="admin-input" placeholder="To date">' +
        '<select id="gens-model" class="admin-input"><option value="">All Models</option><option value="gemini-2.5-flash">Gemini 2.5 Flash</option><option value="gemini-2.0-flash">Gemini 2.0 Flash</option></select>' +
        '<button type="button" class="admin-btn admin-btn--primary admin-btn--sm" id="gens-filter">Filter</button>' +
      '</div>' +
      '<div id="gens-stats" class="admin-gens-stats"></div>' +
      '<div id="gens-list" class="admin-table-wrap"></div>' +
      '<div id="gens-pag" class="admin-pagination"></div>';

    var statsEl = byId('gens-stats');
    var listEl = byId('gens-list');
    var pagEl = byId('gens-pag');

    function load() {
      if (!listEl) return;
      listEl.innerHTML = '<p class="admin-loading">Loading generations...</p>';
      
      var params = { limit: PAGE, offset: offset };
      var searchVal = (byId('gens-search') || {}).value;
      var typeVal = (byId('gens-type') || {}).value;
      var fromVal = (byId('gens-from') || {}).value;
      var toVal = (byId('gens-to') || {}).value;
      var modelVal = (byId('gens-model') || {}).value;
      
      if (searchVal) params.user_id = searchVal;
      if (typeVal) params.feature_type = typeVal;
      if (fromVal) params.from_date = fromVal;
      if (toVal) params.to_date = toVal;
      if (modelVal) params.model = modelVal;
      
      var api = API.admin && API.admin.generations;
      if (!api || !api.list) {
        listEl.innerHTML = '<div class="admin-empty"><p>Generations API not configured yet.</p><p class="admin-empty-hint">This feature requires the backend to expose /api/admin/generations endpoint.</p></div>';
      ...
      api.list(params).then(function(r) {
        ...
        // Show stats
        if (statsEl && r.stats) {
          statsEl.innerHTML = '<div class="admin-stat-cards">' +
            '<div class="admin-stat-card"><span class="admin-stat-num">' + (r.stats.total_generations || 0) + '</span><span class="admin-stat-label">Total</span></div>' +
            '<div class="admin-stat-card"><span class="admin-stat-num">' + (r.stats.scripts_count || 0) + '</span><span class="admin-stat-label">Scripts</span></div>' +
            '<div class="admin-stat-card"><span class="admin-stat-num">' + (r.stats.thumbnails_count || 0) + '</span><span class="admin-stat-label">Thumbnails</span></div>' +
            '<div class="admin-stat-card"><span class="admin-stat-num">$' + (r.stats.total_cost_usd || 0).toFixed(2) + '</span><span class="admin-stat-label">Total Cost</span></div>' +
            '<div class="admin-stat-card"><span class="admin-stat-num">' + ((r.stats.avg_tokens || 0)).toLocaleString() + '</span><span class="admin-stat-label">Avg Tokens</span></div>' +
          '</div>';
          statsEl.style.display = 'block';
        }
        
        if (!items.length) {
          listEl.innerHTML = '<div class="admin-empty"><h3>No generations found</h3><p>Try adjusting your filters.</p></div>';
          if (pagEl) pagEl.innerHTML = '';
          return;
        }
        
        var rows = items.map(function(g) {
          var cost = g.cost_estimate_usd ? '$' + g.cost_estimate_usd.toFixed(4) : '—';
          var tokens = g.total_tokens ? g.total_tokens.toLocaleString() : '—';
          return '<tr><td>' + g.id + '</td><td>' + esc(g.user_id || '—') + '</td><td><span class="admin-badge admin-badge--' + (g.feature_type === 'script' ? 'primary' : 'success') + '">' + esc(g.feature_type || '—') + '</span></td><td>' + esc(g.model_used || '—') + '</td><td>' + tokens + '</td><td>' + cost + '</td><td>' + dateFmt(g.created_at) + '</td></tr>';
        }).join('');
        
        listEl.innerHTML = '<table class="admin-table"><thead><tr><th>ID</th><th>User</th><th>Type</th><th>Model</th><th>Tokens</th><th>Cost</th><th>Time</th></tr></thead><tbody>' + rows + '</tbody></table>';
        
        // Pagination
        if (pagEl) {
          var totalPages = Math.ceil(total / PAGE);
          var currentPage = Math.floor(offset / PAGE) + 1;
          var prevDisabled = offset === 0;
          var nextDisabled = offset + PAGE >= total;
          pagEl.innerHTML = '<span class="admin-pagination-total">' + total + ' total</span>' +
            '<button type="button" class="admin-btn admin-btn--outline admin-btn--sm" id="gens-prev" ' + (prevDisabled ? 'disabled' : '') + '>Previous</button>' +
            '<span class="admin-pagination-info">Page ' + currentPage + ' of ' + totalPages + '</span>' +
            '<button type="button" class="admin-btn admin-btn--outline admin-btn--sm" id="gens-next" ' + (nextDisabled ? 'disabled' : '') + '>Next</button>';
          
          if (byId('gens-prev') && !prevDisabled) byId('gens-prev').onclick = function() { offset = Math.max(0, offset - PAGE); load(); };
          if (byId('gens-next') && !nextDisabled) byId('gens-next').onclick = function() { offset += PAGE; load(); };
        }
      }).catch(function(err) {
        listEl.innerHTML = '<p class="admin-err">' + esc(err.message) + '</p>';
      });
    }
    
    if (byId('gens-filter')) byId('gens-filter').onclick = function() { offset = 0; load(); };
    
    setTimeout(function() {
      if (byId('gens-search')) byId('gens-search').onkeydown = function(e) { if (e.key === 'Enter') { offset = 0; load(); } };
    }, 0);
  ...
  function renderSettings(container) {
    var loadingHtml = '<p class="admin-loading">Loading settings...</p>';
    container.innerHTML = '<div class="admin-page-head"><h1 class="admin-page-title">Settings</h1><p class="admin-page-desc">Configure application settings, feature flags, and AI models.</p></div>' +
      '<div class="admin-settings-tabs">' +
        '<button type="button" class="admin-settings-tab active" data-tab="general">General</button>' +
        '<button type="button" class="admin-settings-tab" data-tab="features">Feature Flags</button>' +
        '<button type="button" class="admin-settings-tab" data-tab="models">AI Models</button>' +
        '<button type="button" class="admin-settings-tab" data-tab="api">API Config</button>' +
      '</div>' +
      '<div id="settings-content" class="admin-settings-content">' + loadingHtml + '</div>';
    
    var contentEl = byId('settings-content');
    var tabs = container.querySelectorAll('.admin-settings-tab');
    ...
    function loadTab(tabName) {
      ...
      if (tabName === 'general') {
        contentEl.innerHTML = '<div class="admin-settings-section">' +
          '<h3 class="admin-settings-section-title">Application Settings</h3>' +
          '<div class="admin-settings-grid">' +
            '<div class="admin-settings-card">' +
              '<h4>App Name</h4><p>Scriptz AI</p>' +
            '</div>' +
            '<div class="admin-settings-card">' +
              '<h4>Version</h4><p>1.0.0</p>' +
            '</div>' +
            '<div class="admin-settings-card">' +
          ...
          '<div class="admin-settings-card admin-settings-card--full">' +
            '<h4>Site URL</h4><input type="text" class="admin-input" value="https://scriptz.app" disabled>' +
          '</div>' +
        ...
        var api = API.admin && API.admin.settings;
        if (!api || !api.getFeatureFlags) {
          contentEl.innerHTML = '<div class="admin-empty"><p>Feature flags API not available.</p></div>';
          return;
        }
        contentEl.innerHTML = '<p class="admin-loading">Loading feature flags...</p>';
        api.getFeatureFlags().then(function(r) {
          var flags = r.feature_flags || {};
          var rows = Object.keys(flags).map(function(k) {
            var enabled = flags[k];
            return '<div class="admin-feature-toggle"><span class="admin-feature-name">' + esc(k) + '</span><label class="admin-toggle"><input type="checkbox" ' + (enabled ? 'checked' : '') + ' data-flag="' + esc(k) + '"><span class="admin-toggle-slider"></span></label></div>';
          }).join('');
          contentEl.innerHTML = '<div class="admin-settings-section"><h3 class="admin-settings-section-title">Feature Flags</h3><div class="admin-feature-list">' + (rows || '<p>No feature flags configured.</p>') + '</div></div>';
        }).catch(function() {
          contentEl.innerHTML = '<div class="admin-empty"><p>Could not load feature flags.</p></div>';
        });
      } else if (tabName === 'models') {
        var api = API.admin && API.admin.settings;
        if (!api || !api.getModels) {
          contentEl.innerHTML = '<div class="admin-empty"><p>Models API not available.</p></div>';
          return;
        }
        contentEl.innerHTML = '<p class="admin-loading">Loading AI models...</p>';
        api.getModels().then(function(r) {
          var models = r.models || [];
          var rows = models.map(function(m) {
            return '<div class="admin-model-card"><div class="admin-model-info"><span class="admin-model-name">' + esc(m.name) + '</span><span class="admin-model-provider">' + esc(m.provider) + '</span></div><label class="admin-toggle"><input type="checkbox" ' + (m.enabled ? 'checked' : '') + ' data-model="' + esc(m.name) + '"><span class="admin-toggle-slider"></span></label></div>';
          }).join('');
          contentEl.innerHTML = '<div class="admin-settings-section"><h3 class="admin-settings-section-title">AI Models</h3><div class="admin-model-list">' + (rows || '<p>No models configured.</p>') + '</div></div>';
        }).catch(function() {
          contentEl.innerHTML = '<div class="admin-empty"><p>Could not load models.</p></div>';
        });
      } else if (tabName === 'api') {
        contentEl.innerHTML = '<div class="admin-settings-section">' +
    ...
    tabs.forEach(function(tab) {
      tab.onclick = function() { loadTab(tab.getAttribute('data-tab')); };
    });
